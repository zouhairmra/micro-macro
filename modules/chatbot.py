...
def run(lang="English"):
    ...
    if st.button("Ask the Bot for a Question" if lang=="English" else "اطلب سؤال من البوت"):
        ...
        if topic_index==0:
            question = "What is opportunity cost?" if lang=="English" else "ما هو تكلفة الفرصة عند اختيار بديل على آخر؟"
            ...
            question = "If supply increases and demand stays same, what happens to price?" if lang=="English" else "إذا زاد العرض وبقي الطلب ثابتًا، ماذا يحدث للسعر؟"
            options = ["Price falls","Price rises","Price unchanged"] if lang=="English" else ["السعر ينخفض","السعر يرتفع","السعر يبقى ثابتًا"]
            answer = options[0]
        elif topic_index==2:
            question = "If demand is inelastic, what happens to total revenue when price rises?" if lang=="English" else "إذا كان الطلب غير مرن، ماذا يحدث للإيرادات الكلية عندما يرتفع السعر؟"
            ...
            options = ["MU decreases as consumption increases","MU increases","MU constant"] if lang=="English" else ["المنفعة الحدية تنخفض مع زيادة الاستهلاك","المنفعة الحدية ترتفع","المنفعة الحدية تبقى ثابتة"]
            answer = options[0]
